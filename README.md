<h1 align="center">
    <img src="https://readme-typing-svg.herokuapp.com/?font=Righteous&size=35&center=true&vCenter=true&width=600&height=70&duration=4000&lines=Hi+There!+👋;+I'm+Kishor+S;Aspiring+Data+Engineer;Building+Scalable+Data+Pipelines;" />
</h1>

<h3 align="center">Aspiring Data Engineer | Big Data | ETL Pipelines | Data Platforms | AI Orchestration</h3>

<p align="center">
  <a href="mailto:kishorsenthilkumar104@gmail.com">Email</a> •
  <a href="https://linkedin.com/in/kishor-s-5b4332274">LinkedIn</a> •
  <a href="https://github.com/Kishorsenthilkumar">GitHub</a>
</p>

---

## 👨‍💻 About Me

🎓 B.Tech in **Artificial Intelligence and Data Science** at Panimalar Engineering College  

🚀 **Aspiring Data Engineer** with hands-on experience building **real-time data pipelines and cloud-based data platforms**  

💡 Passionate about solving **real-world big data problems using scalable architectures**  

📊 Strong foundation in **ETL pipelines, distributed data processing, and data modeling**  

🤖 Exploring **AI Orchestration & LLM-powered data workflows**

---

📄 Resume

👉 (https://raw.githubusercontent.com/Kishorsenthilkumar/kishorsenthilkumar/main/Kishor_Resume%20(2).pdf)

---

---

## 🛠️ Tech Stack

### Programming Languages
<p>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg" width="40"/>
</p>

---

### Data Engineering
<p>
<img src="https://www.vectorlogo.zone/logos/apache_kafka/apache_kafka-icon.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/apache_hadoop/apache_hadoop-icon.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/apache_spark/apache_spark-icon.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/apache_hive/apache_hive-icon.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/apache/apache-original-wordmark.svg" width="40"/>
</p>

---

### Databases
<p>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/postgresql/postgresql-icon.svg" width="40"/>
<img src="https://www.vectorlogo.zone/logos/apache_cassandra/apache_cassandra-icon.svg" width="40"/>
</p>

---

### Cloud & Tools
<p>
<img src="https://cdn.worldvectorlogo.com/logos/aws-2.svg" width="40"/>
<img src="https://cdn.worldvectorlogo.com/logos/snowflake-6.svg" width="40"/>
<img src="https://cdn.worldvectorlogo.com/logos/azure-1.svg" width="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" width="40"/>
</p>

---

### 🤖 AI Orchestration & LLM Tools
<p>
<img src="https://avatars.githubusercontent.com/u/126733545?s=200&v=4" width="40"/>
<img src="https://avatars.githubusercontent.com/u/139895814?s=200&v=4" width="40"/>
</p>

- **LangChain** – Agents, Chains, Retrieval-Augmented Generation (RAG)  
- **LangFlow** – Low-code LLM workflow orchestration  
- **Model Context Protocol (MCP)** – Context-driven AI system integration  

---

## 📊 GitHub Stats

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=Kishorsenthilkumar&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
<img src="https://github-readme-streak-stats.herokuapp.com/?user=Kishorsenthilkumar&theme=tokyonight" />
</p>

<p align="center">
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Kishorsenthilkumar&layout=compact&theme=tokyonight" />
</p>

---

## 🚀 Featured Projects

### 🔹 Real-Time IoT Damage Monitoring Pipeline
- Built a real-time pipeline using **Azure IoT Hub + PySpark Structured Streaming**
- Processed sensor data (vibration, temperature, strain) for damage detection
- Stored results in **PostgreSQL** and visualized via **Grafana**

### 🔹 Real Estate Lakehouse Pipeline
- Implemented **Bronze–Silver–Gold architecture**
- Built **SCD Type 2** historical tracking system
- Automated workflows using **Airflow + Delta Lake**

### 🔹 Real-Time Clickstream Pipeline
- Simulated e-commerce user activity
- Designed analytics-ready schema
- Visualized insights using **Tableau + Cassandra**

---

## 📫 Connect With Me

📧 **Email:** kishorsenthilkumar104@gmail.com  
💼 **LinkedIn:** https://linkedin.com/in/kishor-s-5b4332274  
💻 **GitHub:** https://github.com/Kishorsenthilkumar  

---

⭐ *Building scalable data systems, exploring AI orchestration, and solving real-world problems with data.*
